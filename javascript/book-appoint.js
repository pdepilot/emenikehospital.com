...
const doctors = [
  ...
  },
  {
    ...
    avatar:
      "https://images.unsplash.com/photo-1594824947933-d0501ba2fe65?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80",
  },
  {
    id: 4,
...
function updateSummary() {
  ...
  document.getElementById("summary-service").textContent = serviceSelect.value
    ? serviceSelect.options[serviceSelect.selectedIndex].text
    : "Not selected";

  // Update doctor
  document.getElementById("summary-doctor").textContent = appointmentData.doctor
  ...
  document.getElementById("summary-datetime").textContent =
    dateInput.value && appointmentData.time
      ? `${formatDate(dateInput.value)} at ${appointmentData.time}`
      : "Not selected";
...
document.getElementById("next-3").addEventListener("click", () => {
  ...
  document.getElementById(
    "confirmation-id"
  ).textContent = `AP-${new Date().getFullYear()}-${Math.floor(
    1000 + Math.random() * 9000
  )}`;
  document.getElementById(
    "confirmation-name"
  ).textContent = `${appointmentData.personal.firstName} ${appointmentData.personal.lastName}`;
  ...
  document.getElementById("confirmation-doctor").textContent =
    appointmentData.doctor.name;
  document.getElementById("confirmation-datetime").textContent = `${formatDate(
